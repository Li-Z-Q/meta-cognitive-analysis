...
if __name__ == "__main__":
    ...
    save_path = f'outputs/model_outputs/{model_name}_{dataset_name}_with_procedural_factual_hint_noise.jsonl'
    ...
    for data in datas:
        ...
        noise0 = examples[0]["noisy-factual-hint"] if "noisy-factual-hint" in list(examples[0].keys()) else []
        assert type(noise0) == list
        f_hint0 = examples[0]["factual-hint"] + noise0
        random.shuffle(f_hint0)
        f_hint0 = '\n'.join(f_hint0)
        
        noise1 = examples[1]["noisy-factual-hint"] if "noisy-factual-hint" in list(examples[1].keys()) else []
        assert type(noise1) == list
        f_hint1 = examples[1]["factual-hint"] + noise1
        random.shuffle(f_hint1)
        f_hint1 = '\n'.join(f_hint1)
        
        noise = data["noisy-factual-hint"] if "noisy-factual-hint" in list(data.keys()) else []
